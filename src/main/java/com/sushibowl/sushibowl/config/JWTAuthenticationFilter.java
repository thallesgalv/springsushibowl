...
import com.sushibowl.sushibowl.model.dto.CredenciaisDTO;
...
public class JWTAuthenticationFilter extends UsernamePasswordAuthenticationFilter {
	...
	private RestauranteRepository resRepo;
	
	public JWTAuthenticationFilter(AuthenticationManager authenticationManager, JWTUtil jwtUtil, RestauranteRepository resRepo) {
	...
	protected void successfulAuthentication(HttpServletRequest request, HttpServletResponse response, FilterChain chain, Authentication authResult) throws IOException, ServletException {
		...
		Restaurante res = resRepo.findByLogin(username);
		response.getWriter().append(jsonAuth(token, res));
	}
	...
	private String jsonAuth(String token, Restaurante res) {
		return "{\"token\": \"" + token + "\"" + ", "+
		 "\"username\": " +"\""+ res.getNome() +"\""+ ", " +
		 "\"profile\": " + res.getPerfis().stream().map(x -> "\"" + x + "\"").collect(Collectors.toList()) + "}";
	}
}